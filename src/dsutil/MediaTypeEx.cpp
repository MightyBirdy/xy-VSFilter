...
void CMediaTypeEx::Dump(CAtlList<CString>& sl)
{
	...
	|| formattype == FORMAT_MPEGVideo || formattype == FORMAT_MPEG2_VIDEO)
	{
		...
		if(formattype == FORMAT_VideoInfo2 || formattype == FORMAT_MPEG2_VIDEO)
		{
			...
			sl.AddTail(str);
			str.Format(_T("dwPictAspectRatioX: %u"), vih.dwPictAspectRatioX);
			sl.AddTail(str);
			str.Format(_T("dwPictAspectRatioY: %u"), vih.dwPictAspectRatioY);
			sl.AddTail(str);
		...
		sl.AddTail(str);
		if(bih->biCompression < 256) str.Format(_T("biCompression: %u"), bih->biCompression);
	...
	else if(formattype == FORMAT_WaveFormatEx)
	{
		...
		if(wfe.wFormatTag != WAVE_FORMAT_PCM && wfe.cbSize > 0)
		{
			if(wfe.wFormatTag == WAVE_FORMAT_EXTENSIBLE && wfe.cbSize == sizeof(WAVEFORMATEXTENSIBLE)-sizeof(WAVEFORMATEX))
			{
				...
				if(wfextensible.Format.wBitsPerSample != 0) str.Format(_T("wValidBitsPerSample: %u"), wfextensible.Samples.wValidBitsPerSample);
				else str.Format(_T("wSamplesPerBlock: %u"), wfextensible.Samples.wSamplesPerBlock);
				sl.AddTail(str);
	...
	if(cbFormat > 0)
	{
		...
		for(int i = 0, j = (cbFormat + 15) & ~15; i < j; i += 16)
		{
			...
			for(ptrdiff_t k = i, l = std::min(i + 16l, (long)cbFormat); k < l; k++)
			{
				CString byte;
			...
			for(ptrdiff_t k = std::min(i + 16l, (long)cbFormat), l = i + 16; k < l; k++)
			{
				str += _T("   ");
			}
